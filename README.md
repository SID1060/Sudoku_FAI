...
Choose a Puzzle:
Select a difficulty level from the "DIFFICULTY DATABASE" dropdown or start with an Empty Grid.
...
Board Management:
renderGridSystem(), highlightNode(node, r, c), calculatePossibilities(r, c)

Puzzle Handling:
loadSelectedLevel(), wipeCell()

Solver Algorithms:
runArcConsistency(), runBacktracking(), executeSolver()

Interactivity:
handleExternalInput(val), updateCandidatePanel(r, c)